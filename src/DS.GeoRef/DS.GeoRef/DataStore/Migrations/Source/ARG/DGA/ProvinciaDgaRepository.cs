...
{
    public class ProvinciaDgaRepository : WebApiAsRepository<ProvinciaDto>
    {
        public ProvinciaDgaRepository(string baseUrl, string segment) : base(baseUrl, segment, "", "DatosGobAr")
        {
        ...
        protected override void DoBeforeDeserialization(IRestResponse resp)
        {
            ...
            var provincias = doc.RootElement.GetProperty("provincias");
            var content = provincias.GetRawText();
            resp.Content = content;
        }


        public List<ProvinciaDto> All()
        {
